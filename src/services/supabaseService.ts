...
const mapRowToUser = (row: any): User => ({
  ...
  companyName: row.company_name || 'Adventures',
...
export const saveEmployeeToSupabaseDetailed = async (user: User, previousEmail?: string): Promise<SupabaseSaveResult> => {
  ...
  try {
    ...
    const payload: any = {
      ...
      company_id: user.companyId || 'comp_adventures',
      company_name: user.companyName || 'Adventures',
...
export const fetchEvaluationsFromSupabase = async (employeeIdOrUser?: string | User): Promise<Evaluation[] | null> => {
  ...
  try {
    ...
    const mapped = evals.map((e: any) => ({
      ...
      title: e.title || e.evaluation_title || undefined,
      templateId: e.template_id || e.templateId || 'template_sales',
      templateTitle: e.template_title || e.templateTitle || undefined,
      deploymentId: e.deployment_id || e.deploymentId || undefined,
...
export const saveEvaluationToSupabase = async (evaluation: Evaluation): Promise<boolean> => {
  ...
  try {
    ...
    const payload: Record<string, any> = {
      ...
      title: evaluation.title || undefined,
      template_title: evaluation.templateTitle || undefined,
      deployment_id: evaluation.deploymentId || undefined,
      deadline: evaluation.deadline || undefined,
      department_id: evaluation.departmentId || undefined,
    ...
    if (error && (error.code === 'PGRST204' || error.code === '42703' || error.message.includes('column') || error.message.includes('audit_trail_data') || error.message.includes('kpi_ratings_data') || error.message.includes('full_payload') || error.message.includes('user_id') || error.message.includes('released_by') || error.message.includes('employee_email'))) {
      ...
      const {
        ...
        title, template_title, deployment_id, deadline, department_id,
...
export const saveEvaluationTemplateToSupabase = async (template: EvaluationTemplate): Promise<boolean> => {
  ...
  try {
    ...
    const fullPayload = {
      ...
      templateSource: template.templateSource || 'EMPLOYEE',
      createdForEmployeeId: template.createdForEmployeeId,
      createdForEmployeeName: template.createdForEmployeeName,
      createdForEmployeeNumber: template.createdForEmployeeNumber,
      createdForPosition: template.createdForPosition,
      companyId: template.companyId || 'comp_adventures',
      companyName: template.companyName || 'Adventures',
      immediateSupervisorId: template.immediateSupervisorId,
      immediateSupervisorName: template.immediateSupervisorName,
      isReviewRemarks: template.isReviewRemarks,
      isApprovedAt: template.isApprovedAt,
      podApprovedAt: template.podApprovedAt,
  ...
  try {
    ...
    const templates: EvaluationTemplate[] = rows.map((row: any) => {
      ...
      return {
        ...
        templateSource: full.templateSource || 'EMPLOYEE',
        createdForEmployeeId: full.createdForEmployeeId || undefined,
        createdForEmployeeName: full.createdForEmployeeName || undefined,
        createdForEmployeeNumber: full.createdForEmployeeNumber || undefined,
        createdForPosition: full.createdForPosition || undefined,
        companyId: full.companyId || 'comp_adventures',
        companyName: full.companyName || 'Adventures',
        immediateSupervisorId: full.immediateSupervisorId || undefined,
        immediateSupervisorName: full.immediateSupervisorName || undefined,
        isReviewRemarks: full.isReviewRemarks || undefined,
        isApprovedAt: full.isApprovedAt || undefined,
        podApprovedAt: full.podApprovedAt || undefined,
        revisionHistory: full.revisionHistory || [],
